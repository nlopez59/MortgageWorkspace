...
- [bms/epsmort.bms](../../MortgageApplication/bms/epsmort.bms) is a 3270 screen BMS program written in assembler language.  
  ...
  - The BMS copybook is saved to a PDS using the dbb-zappbuild "HLQ' arg. This PDS is then used as a SYSLIB when EPSCMORT is rebuilt. SYSLIBS are DDnames used by the compiler to find and include copybooks. See the Cobol 'Copy' statement in [epscmort](../../MortgageApplication/cobol/epscmort.cbl#L55).  
    <br />   
    Example of the generated epsmort copybook <img src="../../images/EPSMORT.png#31-34" alt="EPSMORT Copybook" width="500">
...
- [cobol/epscsmrt.cbl](../../MortgageApplication/cobol/epscsmrt.cbl) calculates a mortgage that is passed back to the main EPS**C**MORT program using a COMM-AREA.  In Cobol, COMM-AREAs are data structures define in the 'Linkage Section' to shared data between programs.  
<br />

- [copybook/epsmtcom.cpy](../../MortgageApplication/copybook/epsmtcom.cpy) is the COMM-AREA copybook used by EPSCMORT and EPSCSMRT. This copybook includes 2 other copybooks for input and output data definitions. 
<br />   
...
**CICS Defintions**
- [EPSP](../../WaaS_Setup/initVSI-JCL/dfhcsdup.jcl#L22) is the main MortApp **Transaction ID** (tranid) for the MortApp. All CICS applications must have at least one tranid. 
...
<br />   
- Transactions and all other CICS resources are defined with the IBM batch utility program [DFHCSDUP](https://www.ibm.com/docs/en/cics-ts/6.1?topic=resources-defining-dfhcsdup), as in this example. Or they can be defined with the CICS tranid CEMT [here is a list of other CICS utilities](https://www.tutorialspoint.com/cics/cics_transactions.htm).  In the sample JCL you will see lines (control cards) that define:
...
**DB2 Defintions**
As illustrated below, programs are defined to DB2 using a [Plan](https://www.ibm.com/docs/ru/db2-for-zos/12?topic=recovery-packages-application-plans).  Plans are collections of DB2 packages. A package represents the DB2 resources used by a program.  When a DB2 program is compiled, a DB2 DBRM artifact is create. The DBRM is then bound to DB2 to update its resource requirement before it could run.  
![alt text](image.png)
  
<br />   

- [epsbind.jcl](../../WaaS_Setup/initVSI-JCL/epsbind.jcl#12-17) binds the MortApp and its package. 
    -  The conrtol cards for the bind utility follow "SYSTSIN DD *". 
    -  The DSN control card connects to the DBD1 DB2 subsystem.  
    -  The BIND package point to the PDS member EPSCMORT in the PDS alloacted wit hthe "//DBRMLIB DD ..." card. Thats where DBB stored the DBRM when it built EPSCMORT. 
...
**RACF Security**
RACF is the security subsystem for zOS.     There are others like Top Secret and ACF2. In the MortApp access to it DB2 resources is managed by RACF  

The [racfdef.jcl](../../WaaS_Setup/initVSI-JCL/racfdef.jcl#12) job is run one to define the permission (PE) to access these CICS resoruce.   
 - RDEFINE FACILITY DFHDB2.AUTHTYPE.DBD1 - defines the security profile for the DB2CONN CICS resource created in the DFHCSDUP job above. 
 - RDEFINE FACILITY DFHDB2.AUTHTYPE.EPSE deines security for the DB2ENTRY resource 
 - the 'PE' cards define what RACF User can access these services. 
   - In a WaaS environment the IBMUSER is a special (Root-like) user that is given access. 
   - The CICSUSER is CICS's  Racf ID that is also given access.  
 - The RDELETE cards are used to clean out the defintions when reruning the job. 
...
In our example 
The main programs written in Cobol and the screen program is in assembler - thats the language used for displaying screens. 
